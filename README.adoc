...
Slow producer (server), fast consumer (client). Is the client somehow blocked?
E.g. a thread wasted?
|TODO
|TODO
...
A client zips two Flux from 1 server together. The two Flux
work on a different speed: One Flux works at full speed, the second Flux
emits only one value every 100ms.

Does the fast server Flux eventually react on that and starts producing fewer values per time?
|TODO
|TODO
...
The three process work at different speeds: The client is very slow
