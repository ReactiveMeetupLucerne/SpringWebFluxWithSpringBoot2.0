...
Important techniques were especially the libraries implementing the reactive streams interfaces
http://www.reactive-streams.org: RxJava, Reactor and Akka-Streams.
...
or link:./src/main/java/a_intro/RxJavaCodeExamples.java[RxJavaCodeExamples] for some examples.

=== "Reactive inter-process communication" between two JVM processes

* Reactive hello world server
** link:./src/main/java/b_webflux_helloworld/server/ServerApplication.java[ServerApplication]

* Reactive hello world client
** link:./src/main/java/b_webflux_helloworld/client/ClientApplication.java[ClientApplication]
...
* What are the requirements for a data type to be transported? (e.g. must implement Serializable? or e.g. must follow the JavaBean convention (setter/getter)?)
